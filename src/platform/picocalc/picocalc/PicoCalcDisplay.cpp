...
static inline uint16_t rgb555ToRgb565(uint16_t c) {
    const uint16_t r =  c        & 0x1F;
    const uint16_t g = (c >> 5)  & 0x1F;
    const uint16_t b = (c >> 10) & 0x1F;

    const uint16_t g6 = (g << 1) | (g >> 4);  // expand 5-bit green to 6-bit

    return (r << 11) | (g6 << 5) | b;
}

void PicoCalcDisplay::presentIndexed8(const uint8_t* pixels, int w, int h, const uint16_t* palette) {
    initIfNeeded();

    if (!pixels || !palette || w <= 0 || h <= 0) {
        return;
    }

    if (w != RenderW || h != RenderH) {
        static bool warned = false;
        if (s_serialOutputEnabled && !warned) {
            warned = true;
            std::printf("presentIndexed8: unexpected framebuffer size %dx%d (expected %dx%d)\n",
                        w, h, RenderW, RenderH);
        }
        return;
    }
    ...
    for (int y = 0; y < RenderH; ++y) {
        const uint8_t* srcRow = pixels + y * RenderW;

        for (int x = 0; x < RenderW; ++x) {
            s_present565[x] = rgb555ToRgb565(palette[srcRow[x]]);
        }
