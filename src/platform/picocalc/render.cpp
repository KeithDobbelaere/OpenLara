...
extern RoomRenderRef* gRoomRenderRefs;
extern RoomCacheSlot  gRoomCache[];
extern uint8*         gRoomCacheStorage;
extern uint32         gRoomCacheSlotSize;
extern uint32         gRoomCacheFrameTag;

static constexpr int ROOM_CACHE_SLOTS = 3;

extern uint32 gLevelTilesPsramAddr;
extern uint32 gLevelTilesPsramSize;

extern uint32 gTitleScrPsramAddr;
extern uint32 gTitleScrPsramSize;
extern uint32 gInvBgPsramAddr;
extern uint32 gInvBgPsramSize;

extern uint32 gRoomCacheHits;
extern uint32 gRoomCacheMisses;

extern bool osSnapshotInventoryBackground();
extern const void* osGetInventoryBackgroundHandle();

static const uintptr_t kTitleScrPsramSentinel = 0x5449544Cu; // "TITL"
static const uintptr_t kInvBgPsramSentinel = 0x494E5642u; // "INVB"
...
uint8 fb[FRAME_WIDTH * FRAME_HEIGHT];
...
uint32 gPeakVisibleRooms = 0;
uint32 gCurrentVisibleRooms = 0;

static constexpr uint32 TILE_PAGE_BYTES = 256u * 256u;
static constexpr int TILE_CACHE_SLOTS = 1;

struct TileCacheSlot
{
    uint32 psramAddr;
    uint32 lastUseFrame;
    ALIGN16 uint8 data[TILE_PAGE_BYTES];
};

static TileCacheSlot gTileCache[TILE_CACHE_SLOTS] = {};
static uint32 gTileCacheFrameTag = 1;
static uint32 gTileCacheHits = 0;
static uint32 gTileCacheMisses = 0;

void osResetTileCache()
{
    for (int i = 0; i < TILE_CACHE_SLOTS; i++)
    {
        gTileCache[i].psramAddr = 0xFFFFFFFFu;
        gTileCache[i].lastUseFrame = 0;
    }

    gTileCacheFrameTag = 1;
    gTileCacheHits = 0;
    gTileCacheMisses = 0;
}

static const uint8* getCachedTilePage(uint32 psramAddr)
{
    ASSERT(psramAddr != 0xFFFFFFFFu);

    for (int i = 0; i < TILE_CACHE_SLOTS; i++)
    {
        TileCacheSlot& slot = gTileCache[i];
        if (slot.psramAddr == psramAddr)
        {
            slot.lastUseFrame = gTileCacheFrameTag++;
            gTileCacheHits++;
            return slot.data;
        }
    }

    TileCacheSlot* victim = &gTileCache[0];
    for (int i = 1; i < TILE_CACHE_SLOTS; i++)
    {
        if (gTileCache[i].psramAddr == 0xFFFFFFFFu)
        {
            victim = &gTileCache[i];
            break;
        }

        if (gTileCache[i].lastUseFrame < victim->lastUseFrame)
            victim = &gTileCache[i];
    }

    if (!ol::psram::read(psramAddr, victim->data, TILE_PAGE_BYTES))
    {
        ASSERT(false);
        memset(victim->data, 0, TILE_PAGE_BYTES);
    }

    victim->psramAddr = psramAddr;
    victim->lastUseFrame = gTileCacheFrameTag++;
    gTileCacheMisses++;
    return victim->data;
}
...
{
    uint8* pixel = fb + top->v.y * FRAME_WIDTH;
        ...
        do {
            ...
            if (type <= FACE_TYPE_GTA)
            {
                ...
                if (type > FACE_TYPE_F)
                {
                    ...
                    gTile = getCachedTilePage(tex.tile);
            ...
            else
            {
                ...
                if (type == FACE_TYPE_SPRITE)
                {
                    ...
                    gTile = getCachedTilePage(sprite.tile);
...
void clear()
{
    gCurrentVisibleRooms = 0;
    clearFB((void*)fb);
}

static CachedRoomRenderData getCachedRoomRenderData(int32 roomIndex)
{
    CachedRoomRenderData out = {};

    if (!gRoomRenderRefs)
        return out;

    const RoomRenderRef& ref = gRoomRenderRefs[roomIndex];
    if (ref.psramOffset == 0xFFFFFFFFu || ref.byteSize == 0)
        return out;

    RoomCacheSlot* slot = nullptr;

    for (int i = 0; i < ROOM_CACHE_SLOTS; i++) {
        if (gRoomCache[i].roomIndex == roomIndex) {
            slot = &gRoomCache[i];
            gRoomCacheHits++;
            break;
        }
    ...
    if (!slot)
    {
        gRoomCacheMisses++;
        slot = &gRoomCache[0];
        for (int i = 1; i < ROOM_CACHE_SLOTS; i++) {
            if (gRoomCache[i].lastUseFrame < slot->lastUseFrame)
                slot = &gRoomCache[i];
        }

        ASSERT(ref.byteSize <= gRoomCacheSlotSize);

        if (!ol::psram::read(ref.psramOffset, slot->buffer, ref.byteSize)) {
            ASSERT(false);
            return out;
        }

        slot->roomIndex = roomIndex;
        slot->byteSize = ref.byteSize;
    }

    slot->lastUseFrame = gRoomCacheFrameTag++;

    uint8* ptr = slot->buffer;

    out.verticesCount = ref.verticesCount;
    out.quadsCount = ref.quadsCount;
    out.trianglesCount = ref.trianglesCount;

    out.vertices = (const RoomVertex*)ptr;
    ptr += ref.verticesCount * sizeof(RoomVertex);

    out.quads = (const RoomQuad*)ptr;
    ptr += ref.quadsCount * sizeof(RoomQuad);

    out.triangles = (const RoomTriangle*)ptr;

    return out;
}

static void faceAddRoomCached(const CachedRoomRenderData& rd)
{
    if (rd.quadsCount > 0) {
        ASSERT(rd.quads);
        faceAddRoomQuads(rd.quads, rd.quadsCount);
    }

    if (rd.trianglesCount > 0) {
        ASSERT(rd.triangles);
        faceAddRoomTriangles(rd.triangles, rd.trianglesCount);
    }
}
...
{
    (void)room;
    ASSERT(false); // Old direct room render path must not be used on PicoCalc.
}
...
void renderRoom(Room* room)
{
    ...
    const int32 roomIndex = int32(room - rooms);
    const CachedRoomRenderData rd = getCachedRoomRenderData(roomIndex);
    ...
    gCurrentVisibleRooms++;
    if (gCurrentVisibleRooms > gPeakVisibleRooms)
        gPeakVisibleRooms = gCurrentVisibleRooms;

    {
        PROFILE(CNT_TRANSFORM);
        if (ROOM_FLAG_WATER(room->info->flags)) {
            transformRoomUW(rd.vertices, rd.verticesCount);
        }
        else {
            transformRoom(rd.vertices, rd.verticesCount);
        }
    }

    {
        PROFILE(CNT_ADD);
        faceAddRoomCached(rd);
    }
...
{
    const uintptr_t bg = (uintptr_t)background;

    if (bg == kTitleScrPsramSentinel)
    {
        ASSERT(gTitleScrPsramAddr != 0xFFFFFFFFu);
        ASSERT(gTitleScrPsramSize == 240 * 160);

        constexpr int srcW = 240;
        constexpr int srcH = 160;
        constexpr int dstW = FRAME_WIDTH;
        constexpr int dstH = FRAME_HEIGHT;

        uint8* dst = fb;
        uint8 row[srcW];
        uint32 prevSy = 0xFFFFFFFFu;

        for (int y = 0; y < dstH; y++)
        {
            const uint32 sy = (uint32)((int64_t)y * srcH / dstH);

            if (sy != prevSy)
            {
                ol::psram::read(gTitleScrPsramAddr + sy * srcW, row, srcW);
                prevSy = sy;
            }

            uint8* out = dst + y * dstW;
            for (int x = 0; x < dstW; x++)
            {
                const uint32 sx = (uint32)((int64_t)x * srcW / dstW);
                out[x] = row[sx];
            }
        }
    ...
    if (bg == kInvBgPsramSentinel)
    {
        ASSERT(gInvBgPsramAddr != 0xFFFFFFFFu);
        ASSERT(gInvBgPsramSize == FRAME_WIDTH * FRAME_HEIGHT);
        ol::psram::read(gInvBgPsramAddr, fb, gInvBgPsramSize);
        return;
    }

    if (background)
        dmaCopy(background, fb, FRAME_WIDTH * FRAME_HEIGHT);
}

void* copyBackground()
{
    bool ok = osSnapshotInventoryBackground();
    ASSERT(ok);
    return ok ? (void*)osGetInventoryBackgroundHandle() : nullptr;
}